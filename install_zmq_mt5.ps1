# Script de instalación automática de ZMQ Bridge para MT5
# Bot Trader Copilot v4.11

$ErrorActionPreference = "Stop"

Write-Host "================================================================" -ForegroundColor Cyan
Write-Host "  INSTALADOR AUTOMÁTICO ZMQ BRIDGE PARA MT5" -ForegroundColor Cyan
...
if (-not $MT5Root) {
    Write-Host "ERROR: No se encontró MetaTrader 5" -ForegroundColor Red
...
if (Test-Path $EASource) {
    ...
    Write-Host "   ERROR: No se encontró $EASource" -ForegroundColor Red
    exit 1
}

# Copiar librería JSON
...
if (Test-Path $JSONSource) {
    ...
    Write-Host "   Librería JSON copiada: JAson.mqh" -ForegroundColor Green
} else {
    Write-Host "   ERROR: No se encontró $JSONSource" -ForegroundColor Red
    exit 1
}

# Descargar librería ZMQ para MQL5
Write-Host ""
Write-Host "[3/6] Descargando librería ZMQ para MQL5..." -ForegroundColor Yellow
...
try {
    ...
    Write-Host "   ADVERTENCIA: No se pudo descargar ZMQ automáticamente" -ForegroundColor Yellow
...
if (Test-Path $ConfigFile) {
    ...
    # Habilitar DLL y trading algorítmico
    $modified = $false
    ...
    Write-Host "   Archivo de configuración no encontrado (se configurará manualmente)" -ForegroundColor Yellow
}

# Actualizar config.yaml del bot
Write-Host ""
Write-Host "[5/6] Actualizando configuración del bot..." -ForegroundColor Yellow
...
Write-Host "   [X] Permitir trading algorítmico" -ForegroundColor Yellow
Write-Host "   [X] Permitir importar DLL" -ForegroundColor Yellow
Write-Host ""
Write-Host "6. Arrastra ZMQ_Bridge_EA a cualquier gráfico" -ForegroundColor White
Write-Host ""
Write-Host "7. Configurar parámetros:" -ForegroundColor White
...
Write-Host ""
Write-Host "9. Verificar conexión:" -ForegroundColor White
...
# Abrir MetaEditor si MT5 está instalado
